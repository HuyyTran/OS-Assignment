...
int __alloc(struct pcb_t *caller, int vmaid, int rgid, int size, int *alloc_addr)
{
  ...
  //This get region new limit
    
  caller->mm->symrgtbl[rgid].rg_start = old_sbrk;
  caller->mm->symrgtbl[rgid].rg_end = old_sbrk + size;
  caller->mm->symrgtbl[rgid].rg_next = NULL;

  //This get vm area(address space)
  *alloc_addr = old_sbrk;
...
int __read(struct pcb_t *caller, int vmaid, int rgid, int offset, BYTE *data)
{
  ...
  if (currg->rg_start + offset >= currg->rg_end) {/* Invalid address */
        printf("\t<Process %d> ERROR: Invalid access to memory region=%d offset=%d\n", caller->pid, rgid, offset);
        return -1;
    }  //Updated
...
int pgwrite(
		...
		uint32_t offset)
{
    int ret = __write(proc, 0, destination, offset, data);   //Updated
    if (ret < 0) return -1;   //Updated
#ifdef IODUMP
...
int inc_vma_limit(struct pcb_t *caller, int vmaid, int inc_sz)
{
  ...
  enlist_vm_rg_node(&cur_vma->vm_freerg_list, newrg);  //Updated
